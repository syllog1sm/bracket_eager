...
cdef class MulticlassModel: 
   ...
   cdef load_file(self,fname):
      sys.stderr.write("loading model %s" % fname)
      return self.load(file(fname))

   def from_file(cls,fname):
      return MulticlassModel(file(fname))

   def __init__(self, fh, probs_output=False):
      self.W = {}
